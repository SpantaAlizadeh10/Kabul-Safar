...
export const Footer = () => {
  ...
  return (
    ...
    >
      <div className="flex flex-col items-start gap-4 md:flex-row md:items-start md:justify-between md:gap-8">
        <div className="grid w-full grid-cols-3 gap-2 md:flex-1">
          {content.groups.map((group) => (
            <div key={group.title} className="min-w-0">
              <h3 className="text-[10px] font-bold md:text-sm whitespace-nowrap">
                {group.title}
              </h3>
              <ul className="mt-1 space-y-0 md:mt-2 md:space-y-1">
                {group.links.map((link) => (
                  <li key={link.label}>
                    <a
                      href={link.href}
                      className="text-[8px] text-white/90 transition-colors hover:text-white md:text-xs block truncate"
                    >
                      {link.label}
                    </a>
                  </li>
